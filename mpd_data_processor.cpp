...
void avgB(
    ...
    hls::stream<avg_header_t> &s_avgAHeader,        // read from div
    hls::stream<sample_data_t> &s_avgBSamplesOut,   // write fifo
    hls::stream<avg_pre_header_t> &s_avgBPreHeader, // write div
  ...
  switch(ps)
  {
    ...
    case S_READ_APV_DATA:
      if(!s_avgASamples.empty())
      {
        ...
        static ap_uint<12> binMin[BIN_COUNT], binMax[BIN_COUNT];
        static ap_uint<8> binCnt[BIN_COUNT];
        static ap_uint<20> binSum[BIN_COUNT];

        if(n==0)
        {
          for(int i=0;i<BIN_COUNT;i++)
          {
            binMin[i] = BIN_START+i*BIN_STEP;
    	      binMax[i] = BIN_START+i*BIN_STEP+BIN_WIDTH-1;
            binCnt[i] = 0;
            binSum[i] = 0;
          }
        }
        ...
        for(int i=0;i<BIN_COUNT;i++)
        {
          if(sample_data.data >= binMin[i] && sample_data.data <= binMax[i])
          {
            binCnt[i]++;
            binSum[i]+=sample_data.data;
          }
        }
        ...
        if(n++ == 127)
        {
          int index=0;
          for(int i=0;i<BIN_COUNT;i++)
            if(binCnt[i]>=binCnt[index])
              {
                avg_pre_header.sum=binSum[i];
                avg_pre_header.cnt=binCnt[i];
                index=i;
              }
          ps = S_WRITE_AVGHDR;
        }
...
void avgHeaderDiv(
    hls::stream<avg_pre_header_t> &s_avgAPreHeader, // decoder
    hls::stream<avg_pre_header_t> &s_avgBPreHeader, // read from B
    hls::stream<avg_header_t> &s_avgAHeader,        // write to B
    hls::stream<avg_header_t> &s_avgBHeader         // write to event
  )
{
...
#pragma HLS STREAM variable=s_avgASamples depth=3 type=FIFO
#pragma HLS STREAM variable=s_avgBHeader depth=4 type=FIFO
